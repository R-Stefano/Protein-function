...
class F1MaxScore_tf(tf.keras.metrics.Metric):
	'''
...
class Evaluator():
	...
	def defineGOTermCentricMetric(self):
		...
		self.go_terms_f1max_results_times=[0 for i in self.hyperparams['available_gos']]
		self.go_terms_recall_results_times=[0 for i in self.hyperparams['available_gos']]
		self.go_terms_precision_results_times=[0 for i in self.hyperparams['available_gos']]
